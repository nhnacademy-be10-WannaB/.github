...
서비스 전체 흐름을 처음부터 끝까지 다뤄보는 경험을 할 수 있었습니다.
백엔드에서는 API 설계, DB 모델링, 인증·인가 흐름을 구현하면서, 이전에는 간접적으로만 이해했던 서버와의 상호작용에 대한 기술적 감각과 문제 해결 경험을 쌓았습니다.
또한 GitHub Actions를 활용한 CI/CD 구성, Nginx 설정, 무중단 Rolling 배포, Gateway 및 Eureka 기반의 마이크로서비스 구성 등 인프라 영역까지 다뤄보며, 단순한 기능 구현을 넘어 실제 운영 가능한 서비스를 만드는 과정을 경험했습니다.
